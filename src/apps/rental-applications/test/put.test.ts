...
describe('Application submission PUT Requests', () => {
    test('Responds with Method not allowed, status 405: User defined Ids not allowed', 
        async() =>{
            const response = await request(app).put('/application-submission')
                .send(postData)
    ...
    test('Responds with validation errors, status 400: Ivalid assetId', 
        async() => {
            const response = await request(app).put('/application-submission/64c9e4f2df7cc0tgd')
                .send(postData)
    ...
    test('Responds with validation errors, status 400: Invalid input data.', 
        async() =>{
            const response = await request(app).put('/application-submission/64c9e4f2df7cc072af2ac9e4')
                .send(invalidData)
    ...
    test('Responds with created resource, status 201: New document created.',
        async() =>{
            const response = await request(app).put('/application-submission/64c9e4f2df7cc072af2ac9e2')
                .send(postData)
