...
describe('Application Submission PATCH route', () => {
    test('Responds with Method not allowed, status 405: Rejects batch patch request',
        async() => {
            const response = await request(app).patch('/application-submission').send(postData)
    ...
    test('Responds with validation errors, status 400: Ivalid application Id', 
        async() => {
            const response = await request(app).patch('/application-submission/64c9e4f2df7cc0tgd')
    ...
    test('Responds with validation errors, status 400: Invalid input.', 
        async() =>{
            const response = await request(app).patch('/application-submission/64c9e4f2df7cc072af2ac9e4')
                .send(invalidData)
    ...
    test('Responds with Not Found, status 404: Target does not exist', 
        async() => {
            const response = await request(app).patch('/application-submission/64c9e4f2df7cc072af2ac9e8')
                .send(postData)
